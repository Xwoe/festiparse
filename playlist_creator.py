...
class PlaylistCreator:
    ...
    def get_all_playlist_tracks(self, playlist_id):
        ...
        items = self.spotify.user_playlist_tracks(
            self.username, playlist_id, offset=0, fields=None, limit=stepsize
        )["items"]
        while len(items) > 0:
    ...
    def get_or_create_playlist(self):
        ...
        return self.spotify.user_playlist_create(
            self.username, self.playlist_name, self.playlist_description
        )["id"]
    ...
    def add_tracks(self, album_tracks, artist):
        ...
        if self.playlist_tracks:
            add_tracks = [
                track for track in album_tracks if track not in self.playlist_tracks
            ]
        else:
            add_tracks = album_tracks
        if len(add_tracks) > 0:
            self.spotify.user_playlist_add_tracks(
                self.username, self.playlist_id, add_tracks, position=None
            )
